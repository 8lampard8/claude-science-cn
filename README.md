...
如果说 Claude Code 改变的是程序员写代码的方式，那么 Claude Science 瞄准的就是科研工作流本身。它不是一个普通聊天窗口，而是 Anthropic 面向科学研究推出的本地科研工作台：可以运行 Python / R / shell 分析流程，连接科学数据库和实验室工具，管理本地、服务器或 HPC 上的计算环境，并把数据处理、代码、图表、对话和结果溯源保存在同一个项目里。

对科研用户来说，这一点非常关键。传统 AI 聊天工具能帮你解释概念、改代码、写摘要，但一旦进入真实科研流程，问题就会变成：数据在哪里？代码怎么跑？图表怎么复现？引用是否可靠？几个月后还能不能追溯这张图是怎么生成的？Claude Science 的价值正在这里：它把“问答式 AI”推进到“可执行、可复现、可审计”的科研工作台。

官方介绍里，Claude Science 支持单细胞 RNA-seq、进化分析、蛋白结构、化学信息学等生命科学场景；可以查询 60+ 科学数据库，管理 Python 和 R 内核，渲染蛋白、分子结构、基因组轨道和 PDF，并让每个图表、表格、notebook 都带上生成它的代码、环境和对话记录。换句话说，它强的不是“帮你写一段回答”，而是能陪你从文献调研、数据清洗、建模分析、画图迭代，一直走到论文草稿和结果复盘。

但问题也很现实：Claude Science 在国内并不好用。

第一，访问门槛高。Claude Science 目前是 beta，官方支持 macOS 和 Linux，并面向 Pro、Max、Team、Enterprise 等付费计划开放。按照 Claude 官方价格页，Pro 月付为 20 美元，Max 从 100 美元/月起，Team 也按席位收费。对个人研究者、学生、小课题组来说，这已经不是一个“顺手试试”的成本。

第二，账号和地区限制更麻烦。国内用户访问 Claude 相关服务本身就不稳定，注册、登录、支付、风控都可能卡住；企业和机构用户还会遇到更严格的主体限制。近期香港、海外主体、云服务中转等方式也不再稳定，Anthropic 对来自中国相关主体的访问管控越来越严格。很多人不是不想付费，而是连稳定进入官方体系都很难。

这就造成了一个很尴尬的局面：Claude Science 恰恰是科研用户最需要的那类 AI 工具，但国内用户最难稳定、低成本、合规地使用它。本文的重点，正是围绕这个现实问题展开：在理解 Claude Science 科研价值的前提下，梳理它的能力边界、部署门槛，以及国内研究者可以考虑的本地化、BYOK 或替代工作流方案。
